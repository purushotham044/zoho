/**
 * Create artifact-fixed.zip with exact structure for Catalyst deployment
 * Root level files: catalyst-config.json, index.js, package.json, package-lock.json
 * catalyst/ folder with functions and lib
 */
...
console.log('📦 Creating artifact-fixed.zip with exact structure...\n');

// Output zip path
const outputZip = path.join(__dirname, '..', 'artifact-fixed.zip');

// Remove old zip if exists
if (fs.existsSync(outputZip)) {
  fs.unlinkSync(outputZip);
  console.log('🗑️  Removed existing artifact-fixed.zip\n');
}
...
// 1. Add catalyst-config.json at root (from catalyst/catalyst.json)
const catalystConfigPath = path.join(rootDir, 'catalyst', 'catalyst.json');
if (fs.existsSync(catalystConfigPath)) {
  ...
  console.error('❌ catalyst/catalyst.json not found!');
  process.exit(1);
}
...
// 8. Optionally add README.md if it exists
