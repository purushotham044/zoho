/**
 * Create artifact-final-fixed.zip with exact structure for Catalyst deployment
 * Root level files: catalyst-config.json, index.js, package.json, package-lock.json
 * catalyst/ folder with functions, lib, scheduler.json
 * NO node_modules included
 * NO parent folder wrapping
 */
...
console.log('📦 Creating artifact-final-fixed.zip with exact structure...\n');

// Output zip path
const outputZip = path.join(__dirname, '..', 'artifact-final-fixed.zip');

// Remove old zip if exists
if (fs.existsSync(outputZip)) {
  fs.unlinkSync(outputZip);
  console.log('🗑️  Removed existing artifact-final-fixed.zip\n');
}
...
output.on('close', () => {
  ...
  console.log('📤 Ready to upload to Catalyst → Functions → Update Function!\n');
});
...
if (fs.existsSync(indexPath)) {
  ...
  console.log('✅ Added index.js (from catalyst/index.js)');
} else {
...
if (fs.existsSync(functionsDir)) {
  // Use directory method with filter to exclude node_modules
  const addDirectory = (dir, zipPath) => {
    ...
    entries.forEach(entry => {
      ...
      if (entry.isDirectory()) {
        ...
        // Skip files in node_modules paths
  ...
  addDirectory(functionsDir, 'catalyst/functions');
...
if (fs.existsSync(libDir)) {
  ...
  addDirectory(libDir, 'catalyst/lib');
