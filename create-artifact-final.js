/**
 * Create artifact-final.zip with exact structure for Catalyst deployment
...
console.log('📦 Creating artifact-final.zip with exact structure...\n');

// Output zip path
const outputZip = path.join(__dirname, '..', 'artifact-final.zip');

// Remove old zip if exists
if (fs.existsSync(outputZip)) {
  fs.unlinkSync(outputZip);
  console.log('🗑️  Removed existing artifact-final.zip\n');
}
...
if (fs.existsSync(functionsDir)) {
  archive.directory(functionsDir, 'catalyst/functions', {
    filter: (entry) => {
...
if (fs.existsSync(libDir)) {
  archive.directory(libDir, 'catalyst/lib', {
    filter: (entry) => {
      // Exclude node_modules
      return !entry.includes('node_modules');
    }
